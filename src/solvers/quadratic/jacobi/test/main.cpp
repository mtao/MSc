...
#include <sys/time.h>
#include <iostream>
#include "solvers/quadratic/jacobi/jacobi.hpp"
#include <vector>
...
void run_dense_test(int dim)
{
    ...
    DenseJacobiSolve(A,b,x);
    //DenseJacobiSubspaceMinimizationSolve(A,b,x);
...
void run_sparse_test(int dim)
{
    ...
    auto sln(x);
    x = decltype(x)::Zero(x.rows(),x.cols());
    SparseJacobiSolve(A,b,x);
    //SparseJacobiSubspaceMinimizationSolve(A,b,x);
