...
#include <Eigen/Dense>
#include "../linear.h"
#include "solvers/cholesky/ldlt.hpp"



template <typename MatrixType, typename VectorType, typename Preconditioner>
struct PreconditionedConjugateGradientCapsule: public SolverCapsule<MatrixType, VectorType>
{
    ...
    Scalar error(){return r.squaredNorm();}
    void step()
    {
        alpha = (rdz)/(p.dot(Ap));
        x+=alpha * p;
...
void PreconditionedConjugateGradientSolve(const Matrix & A, const Vector & b, Vector & x)
{
    ...
    auto solver = IterativeSolver<PreconditionedConjugateGradientCapsule<Matrix,Vector, Preconditioner> >(5*A.rows(), 1e-5*residual);
    //auto solver = IterativeSolver<PreconditionedConjugateGradientCapsule<Matrix,Vector, Preconditioner> >(A.rows(), 1e-5);
    solver.solve(A,b,x);
}
