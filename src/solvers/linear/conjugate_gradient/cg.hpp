...
struct ConjugateGradientCapsule: public SolverCapsule<MatrixType, VectorType>
{
...
void ConjugateGradientSolve(const Matrix & A, const Vector & b, Vector & x)
{
    ...
    auto solver = IterativeSolver<ConjugateGradientCapsule<Matrix,Vector> >(A.rows(), 1e-5*residual);
    solver.solve(A,b,x);
}
