...
PointViewerWidget::PointViewerWidget(const QGLFormat & format, const QString & prefix, QWidget * parent): ObjectViewerWidget(format,prefix,parent),
...
void PointViewerWidget::sendBuffers()
{
    ...
    m_shader->bind();
    if(prepareBufferObject(m_vertexBuffer, QGLBuffer::StaticDraw, m_points.data(), sizeof(Vector3)*m_points.size()))
        setAttributeObject("vertex", GL_FLOAT, 0, 3);
    else
        return;
    m_shader->setUniformValue( "u_color", m_color );
    m_shader->release();
    ObjectViewerWidget::sendBuffers();
