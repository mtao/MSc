...
BloatViewer::BloatViewer(const QString & prefix , const QString & file_extension, QWidget * parent, Qt::WFlags flags): Viewer<BloatViewerWidget>(prefix,file_extension,parent,flags){init();}
void BloatViewer::init()
{
    control=0;
    shaderName=0;
...
void BloatViewer::createControls()
{
    if(control)
        delete control;
    ...
    QSlider * bloatSlider = new QSlider();
    connect(bloatSlider, SIGNAL(valueChanged(int)), m_widget, SLOT(bloatChange(int)));
    control->layout()->addWidget(bloatSlider);

}

void BloatViewer::newShaderPrefix()
{
